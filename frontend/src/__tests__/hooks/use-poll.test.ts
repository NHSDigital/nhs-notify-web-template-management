...
function createOptions(
  ...
  return {
    fetchFn: jest
      .fn<Promise<PollValue | null>, [AbortSignal]>()
...
describe('usePoll', () => {
  ...
  it('starts in polling state when initialValue is omitted', () => {
    const { result } = renderHook(() =>
      usePoll(createOptions({ initialValue: undefined }))
    );
  ...
  it('does not call shouldPoll during init when initialValue is omitted', () => {
    ...
    renderHook(() =>
      usePoll(
        createOptions({ initialValue: undefined, shouldPoll: shouldPollSpy })
      )
    );
  ...
  it('stops polling when onError returns true', async () => {
    const networkError = new TypeError('Failed to fetch');
    const fetchFn = jest
      .fn<Promise<PollValue | null>, [AbortSignal]>()
      .mockRejectedValue(networkError);
    ...
    // Interval would fire here if still polling — it must not
    await act(async () => {
      jest.advanceTimersByTime(1000);
    });
    expect(fetchFn).toHaveBeenCalledTimes(1);
  ...
  it('continues polling when onError returns false', async () => {
    const networkError = new TypeError('Failed to fetch');
    const fetchFn = jest
      .fn<Promise<PollValue | null>, [AbortSignal]>()
      .mockRejectedValue(networkError);
  ...
  it('does not throw when onError is omitted and fetchFn throws', async () => {
    ...
    const { result } = renderHook(() => usePoll(createOptions({ fetchFn })));
  ...
  it('clears timeout timer on unmount when timeoutMs is set', async () => {
    ...
    const { unmount } = renderHook(() =>
      usePoll(createOptions({ timeoutMs: 20_000 }))
    );
  ...
  it('does not call onUpdate when fetch resolves after timeout', async () => {
    ...
    const { result } = renderHook(() =>
      usePoll(
        createOptions({ fetchFn, onUpdate, timeoutMs: 2000, intervalMs: 5000 })
      )
    );
