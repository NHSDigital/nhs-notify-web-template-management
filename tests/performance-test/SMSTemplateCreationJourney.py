...
class SMSCreate(PlaywrightUser):
    ...
    async def WebTemplate(self, page: PageWithRetry):
        try:
            ...
            async with event(self, "3_Select SMS And Contime"):  ## log this as third event in journey
                async with page.expect_navigation(wait_until="domcontentloaded"):
                    await self.page.get_by_test_id("SMS-radio").check()  #select NHSApp Template
            ...
            async with event(self, "4_Populate SMS Template"):  ## log this as fourth event in journey
                async with page.expect_navigation(wait_until="domcontentloaded"):
                    ...
                    await self.page.get_by_test_id("sms-submit-radio").check()
            async with event(self, "5_Submit SMS Template"):  ## log this as fifth and final event in journey
                async with page.expect_navigation(wait_until="domcontentloaded"):
