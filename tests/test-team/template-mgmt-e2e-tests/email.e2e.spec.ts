...
const createAndPreviewEmailTemplate = async (page: Page, userId: string) => {
...
test('Email e2e test - routing disabled', async ({ page }) => {
  await createAndPreviewEmailTemplate(page, testUsers.User2.userId);
...
test('Email e2e test - routing enabled', async ({ page }) => {
  await createAndPreviewEmailTemplate(page, testUsers.User1.userId);
