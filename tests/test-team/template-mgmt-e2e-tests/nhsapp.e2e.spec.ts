...
const createAndPreviewNhsAppTemplate = async (page: Page, userId: string) => {
...
test('NHS App e2e test - routing disabled', async ({ page }) => {
  await createAndPreviewNhsAppTemplate(page, testUsers.User2.userId);
...
test('NHS App e2e test - routing enabled', async ({ page }) => {
  await createAndPreviewNhsAppTemplate(page, testUsers.User1.userId);
