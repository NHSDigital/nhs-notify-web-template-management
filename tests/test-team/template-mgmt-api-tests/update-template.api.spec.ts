...
test.describe('POST /v1/template/:templateId', () => {
  ...
  test('returns 400 if template status is invalid', async ({ request }) => {
    ...
    expect(await updateResponse.json()).toEqual({
      ...
      details: {
        templateStatus:
          "Invalid enum value. Expected 'NOT_YET_SUBMITTED' | 'SUBMITTED' | 'DELETED' | 'PENDING_UPLOAD' | 'PENDING_VALIDATION' | 'VIRUS_SCAN_FAILED' | 'VALIDATION_FAILED', received 'INVALID'",
      },
    });
  });
