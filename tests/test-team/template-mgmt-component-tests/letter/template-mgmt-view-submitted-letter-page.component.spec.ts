...
function createTemplates(owner: string) {
  return {
    ...
    invalid: {
      ...TemplateFactory.createLetterTemplate(
        'invalid-letter-template-view-submitted',
        owner,
        'invalid-letter-template-view-submitted'
      ),
      name: 'test-template-letter',
...
test.describe('View submitted Letter message template Page', () => {
  ...
  test('when user visits page, then page is loaded', async ({
    ...
    const viewSubmittedLetterTemplatePage =
      new TemplateMgmtViewSubmittedLetterPage(page);

    await viewSubmittedLetterTemplatePage.loadPage(templates.valid.id);

    await expect(page).toHaveURL(
      `${baseURL}/templates/view-submitted-letter-template/${templates.valid.id}`
    );

    await expect(viewSubmittedLetterTemplatePage.pageHeader).toContainText(
  ...
  test.describe('Error handling', () => {
    test('when user visits page with an unsubmitted template, then an invalid template error is displayed', async ({
      ...
      await viewSubmittedLetterTemplatePage.loadPage(templates.invalid.id);
    ...
    test('when user visits page with a fake template, then an invalid template error is displayed', async ({
      ...
      await viewSubmittedLetterTemplatePage.loadPage('/fake-template-id');
