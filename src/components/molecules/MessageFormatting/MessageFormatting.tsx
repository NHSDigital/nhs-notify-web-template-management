...
export function MessageFormatting({ template }: MessageFormattingType) {
  return (
    <>
      ...
      {template === TemplateFormatText.APP ? AppFormatting() : null}
      {template === TemplateFormatText.EMAIL ? EmailFormatting(template) : null}
      {template === TemplateFormatText.LETTER
        ? LetterFormatting(template)
        : null}
      {template === TemplateFormatText.SMS ? SMSFormatting() : null}
    </>
  );
}
  ...
  return (
    <Details data-testid='link-and-url-details'>
      ...
      <Details.Text data-testid='link-and-url-text'>
        ...
        <code>{messageFormattingContent.linksAndUrls.codeBlockText.text2}</code>
      </Details.Text>
    </Details>
  );
};
...
const EmailFormatting = (template: string) => {
  return (
    <>
      {AppFormatting()}
...
const LetterFormatting = (template: string) => {
  return (
    <>
      {EmailFormatting(template)}
