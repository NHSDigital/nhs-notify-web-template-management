...
describe('configReader', () => {
  ...
  describe.each([config.getInt, config.tryGetInt])('%p', (func) => {
    ...
    // It is safer to throw for a malformed value as this is more likely to be a configuration
    // error than an attempt to not pass a value. Even for tryGetInt.
  ...
  describe.each([config.getBoolean, config.tryGetBoolean])('%p', (func) => {
    ...
    // It is safer to throw for a malformed value as this is more likely to be a configuration
    // error than an attempt to not pass a value. Even for tryGetBoolean.
    it('will throw if value cannot be coerced to boolean', () => {
      expect(() => func('A_FLOAT')).toThrow(
        'A_FLOAT is not valid. Expected true or false but got 3.14159265359'
      );
    });
  });

  describe('fluent interface', () => {
    it('can validate a minimum integer', () => {
      expect(
        config.read('AN_INT').asInt().validateGreaterThan(461).value()
      ).toBe(462);
    });

    it('can validate a minimum integer, throws if too small', () => {
      expect(() =>
