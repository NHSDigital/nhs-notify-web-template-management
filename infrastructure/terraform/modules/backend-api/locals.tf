locals {
  ...
  openapi_spec = templatefile("${path.module}/spec.tmpl.json", {
    ...
    CREATE_CONTACT_DETAILS_LAMBDA_ARN             = module.create_contact_details_lambda.function_arn
    CREATE_ROUTING_CONFIG_LAMBDA_ARN              = module.create_routing_config_lambda.function_arn
    CREATE_TEMPLATE_LAMBDA_ARN                    = module.create_template_lambda.function_arn
    ...
    SUBMIT_TEMPLATE_LAMBDA_ARN                    = module.submit_template_lambda.function_arn
  ...
  backend_lambda_environment_variables = {
    ...
    EVENT_TOPIC_ARN                         = var.events_sns_topic_arn
