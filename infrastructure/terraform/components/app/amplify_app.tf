resource "aws_amplify_app" "main" {
  ...
  environment_variables = {
    ...
    NEXT_PUBLIC_COGNITO_USER_POOL_ID        = jsondecode(data.aws_ssm_parameter.cognito_config.value)["USER_POOL_ID"]
    NEXT_PUBLIC_COGNITO_USER_POOL_CLIENT_ID = jsondecode(data.aws_ssm_parameter.cognito_config.value)["USER_POOL_CLIENT_ID"]
  }
}
