...
### POST - /v1/letter-template - Create a DOCX letter template

Will create a single letter template. Do not set a content type header as this must be auto-generated by curl, so that it includes a form-data boundary.

```bash
DOCX_PATH="./tests/test-team/fixtures/letters/docx/standard-english-template.docx"
```
...
### POST - /v1/letter-template - Create a PDF letter template
